 function drawAssignment(x, y, artifact, scale, draw) {
            ...
            p.click(function() {
				...
				var grade = prompt("Please enter a What-IF Grade", "%");
				if (grade == null || grade == "") {
			...
			p.mouseover(function() {
				...
				text = draw.text("Grade: " + grade*100 + "%, Weight: " + weight + ", Average: " + average*100 + "%");
				text.x(scale*1.1);
				text.y(0);
				text.addClass('temp');
				text.font({
		...
		function drawPrediction(x, y, data, scale, draw, percentage) { //This could probably be done better, but works 
			...
			if (startPointFound) {
				...
				MaxLine.mouseover(function() {
					...
					text = draw.text("Max Grade: " + (maxGrade*100).toPrecision(3) + "%");
					text.x(scale*1.1);
					text.y(0);
					...
					text = draw.text("Projection Grade: " + (projection*100).toPrecision(3) + "%");
					text.x(scale*1.1);
					text.y(scale/25);
					text.addClass('projectionText');
					text.font({
					...
					text = draw.text("Minimum Grade: " + (minimum*100).toPrecision(3) + "%");
					text.x(scale*1.1);
					text.y(scale*2/25);
					text.addClass('projectionText');
					text.font({
		...
		function drawAverageGrades(x, y, artifact, scale, draw) {
            let x1 = x;
            let y1 = y;
            let x2 = x1 + (artifact.weight * scale/100);
            let y2 = y1 - (artifact.averageScore * artifact.weight)*scale/100;
            let lineString = `${x1},${scale} ${x1},${y1} ${x2},${y2} ${x2},${scale}`;
		...
		function drawLegend(scale,draw,fontsize) {
			...
			//Label "Legend
			y = 5;
			drawText(x,y,"Legend",fontsize*1.4,draw);
			...
			drawText(x+scale/12+10,y-scale/120,"Your Current/Min Grade", fontsize, draw);
			...
			drawText(x+scale/12+10,y-scale/120,"Class Average", fontsize, draw);
			...
			drawText(x+scale/12+10,y-scale/120,"Your Potential/Max Grade", fontsize, draw);
			
			//Your Predicted Grade
			y = y + scale/20;
			let predictionLine = draw.line([[x,y+10],[x+scale/12,y+10]]).stroke({color: 'blue', width: 3, dasharray: '5,5'});
			drawText(x+scale/12+10,y-scale/120,"Your Predicted Grade", fontsize, draw);
		...
		function drawChart(scale, svg){
			...
			drawText(buttonx+ scale*.01,buttony-scale*.01, "Reset Data", scale/25, draw);
